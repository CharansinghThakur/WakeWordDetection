import torch
from torch import nn
from torch.utils.data import DataLoader
import torchaudio
import pandas as pd
import numpy as np
from sonopy import power_spec, mel_spec, mfcc_spec, filterbanks

class MFCC(nn.Module):
    
    def __init__(self, sample_rate, fft_size=400, window_stride=(400, 200), num_filt=40, num_coeffs=40, max_samples = 128):
        super(MFCC, self).__init__()
        self.sample_rate = sample_rate
        self.fft_size = fft_size
        self.num_filt = num_filt
        self.num_coeffs = 40
        self.window_stride = window_stride
        self.max_samples = max_samples

        self.mfcc = lambda x: mfcc_spec(x, self.sample_rate, self.window_stride,
                self.fft_size, self.num_filt, self.num_coeffs
            )
    
    def forward(self, x):
        mfcc_out = self.mfcc(x.squeeze(0).numpy()) # calculate the mfcc features
        
        if mfcc_out.shape[0] < self.max_samples:
            # Padding at the end
            padding = ((0, self.max_samples - mfcc_out.shape[0]), (0, 0))
            padded_mfcc = np.pad(mfcc_out, padding, mode='constant')
        else:
            # Truncate from the bottom
            padded_mfcc = mfcc_out[:self.max_samples, :]
        return torch.Tensor(padded_mfcc).transpose(0, 1).unsqueeze(0)
    
def get_features(sample_rate):
    return MFCC(sample_rate = sample_rate)

class RandomCut(nn.Module):
    """Augmentation technique that randomly cuts start or end of audio"""

    def __init__(self, max_cut=10):
        super(RandomCut, self).__init__()
        self.max_cut = max_cut

    def forward(self, x):
        """Randomly cuts from start or end of batch"""
        side = torch.randint(0, 1, (1,))
        cut = torch.randint(1, self.max_cut, (1,))
        if side == 0:
            return x[:-cut,:,:]
        elif side == 1:
            return x[cut:,:,:]
        
class SpecAugment(nn.Module):

    def __init__(self, rate, policy=3, freq_mask = 2, time_mask = 4):
        super(SpecAugment, self).__init__()

        self.rate = rate

        self.specaug = nn.Sequential(
            torchaudio.transforms.FrequencyMasking(freq_mask_param=freq_mask),
            torchaudio.transforms.TimeMasking(time_mask_param=time_mask)
        )

        self.specaug2 = nn.Sequential(
            torchaudio.transforms.FrequencyMasking(freq_mask_param=freq_mask),
            torchaudio.transforms.TimeMasking(time_mask_param=time_mask),
            torchaudio.transforms.FrequencyMasking(freq_mask_param=freq_mask),
            torchaudio.transforms.TimeMasking(time_mask_param=time_mask)
        )

        policies = {1: self.policy1, 2:self.policy2, 3:self.policy3}

        self._forward = policies[policy]

    def forward(self, x):
        return self._forward(x)

    def policy1(self, x):
        probability = torch.rand(1,1).item()
        if self.rate > probability:
            return self.specaug(x)
        else:
            return x
    
    def policy2(self, x):
        probability = torch.rand(1,1).item()
        if self.rate > probability:
            return self.specaug2(x)
        else:
            return x
    
    def policy3(self, x):
        probability = torch.rand(1,1).item()

        if probability > 0.5:
            return self.policy1(x)
        else:
            return self.policy2(x)
        
class WakeWordDataset(torch.utils.data.Dataset):
    def __init__(self, json_data, sample_rate = 8000, valid = False, max_samples=128):
        self.sr = sample_rate
        self.data = pd.read_json(json_data, lines = True)
        if valid:
            self.audio_transform = get_features(sample_rate)
        else:
            self.audio_transform = nn.Sequential(
                get_features(sample_rate),
                SpecAugment(rate = 0.5)
            )
    
    def __len__(self):
        return len(self.data)
    
    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.item()

        try:
            file_path = self.data.audio_path.iloc[idx]

            waveform, sr = torchaudio.load(file_path)
            if sr > self.sr:
                waveform = torchaudio.transforms.Resample(sr, self.sr)(waveform)
            
            mfcc = self.audio_transform(waveform)

            label = self.data.label.iloc[idx]
            
        except Exception as e:
            print(str(e))
            return self.__getitem__(torch.randint(0, len(self), (1,)))
    
        return mfcc, label

rand_cut = RandomCut(max_cut=10)


def collate_fn(data):
    """Batch and pad wakeword data"""
    mfccs = []
    labels = []
    for d in data:
        mfcc, label = d
        # mfccs.append(mfcc.squeeze(0).transpose(0, 1))
        mfccs.append(mfcc)
        labels.append(label)

    # pad mfccs to ensure all tensors are same size in the time dim
    # mfccs = nn.utils.rnn.pad_sequence(mfccs, batch_first=True)  # batch, seq_len, feature
    mfccs = mfccs.transpose(0, 1) # seq_len, batch, feature
    mfccs = rand_cut(mfccs)
    #print(mfccs.shape)
    labels = torch.Tensor(labels)
    return mfccs, labels

if __name__ == '__main__':
    train_data = WakeWordDataset(json_data='json_training_dir/train_data.json')
    valid_data = WakeWordDataset(json_data='json_training_dir/test_data.json')

    train_data_loader = DataLoader(train_data,
                                   batch_size = 16,
                                   shuffle = True)
    valid_data_loader = DataLoader(valid_data,
                                   batch_size = 1,
                                   shuffle = True)


    print("MFCC Feature Shape:" , train_data[67][0].shape)

    for batch in train_data_loader:
        print(batch[0].shape)
        break


                                   